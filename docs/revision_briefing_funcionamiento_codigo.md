...
### Resuelto - Entornos Python locales rotos eliminados
...
- Las carpetas locales `.venv` y `venv` se han eliminado porque estaban rotas y se pueden recrear desde `requirements.txt`.
- Para uso normal fuera de Codex, sigue siendo recomendable recrear un unico entorno virtual.
...
### Resuelto - `notebooks/prueba.py` eliminado

El archivo era una prueba exploratoria de rutas y no formaba parte de la tuberia final documentada. Se ha eliminado en la limpieza del proyecto.
